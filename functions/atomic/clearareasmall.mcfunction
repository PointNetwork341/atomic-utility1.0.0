...
execute @p[scores={cleararea=2},tag=stafftag] ~~~ tellraw @a {"rawtext":[{"text":"§¶§cAtomic Utility §b⤤ §d"},{"selector":"@s"},{"text":" §¶§bjust cleared a small area"}]}
kill @e[type=armor_stand,name=cleararea]

#Make-Shift "are you sure"
execute @p[scores={cleararea=1},tag=stafftag] ~~~ tellraw @p[tag=!clearcheck] {"rawtext":[{"text":"§¶§cAtomic⤤ This will fill all blocks within a 15 radius with air!"}]}
execute @p[scores={cleararea=1},tag=stafftag] ~~~ tellraw @p[tag=!clearcheck] {"rawtext":[{"text":"§¶§cAtomic⤤ This can't be fixed! Execute command again if you're sure."}]}
execute @p[scores={cleararea=2},tag=stafftag] ~~~ tellraw @p[tag=!clearcheck] {"rawtext":[{"text":"§¶§cAtomic⤤ The surrounding area has been cleared with air."}]}
