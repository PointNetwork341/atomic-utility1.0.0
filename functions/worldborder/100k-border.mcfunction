...
#Deny Nonstaff
execute @s[tag=!stafftag] ~~~ tellraw @s[scores={wbmtoggle=1}] {"rawtext":[{"text":"§¶§cAtomic Utility §¶§b⤤ Access §cDENIED§7! §bOnly staff can use this command"}]}
execute @s[tag=!stafftag] ~~~ execute @s ~~~ playsound note.bass @s ~ ~ ~
execute @s[tag=stafftag] ~~~ execute @s ~~~ playsound note.pling @s ~ ~ ~
execute @s[tag=stafftag] ~~~ tellraw @a[tag=stafftag,scores={wbmtoggle=1}] {"rawtext":[{"text":"§¶§cAtomic Utility §¶§b⤤ §6World Border §bhas been set to §2100k §bby §d"},{"selector":"@s"}]}

#check for toggle
execute @s[tag=stafftag] ~~~ tellraw @s[scores={wbmtoggle=0}] {"rawtext":[{"text":"§¶§cAtomic Utility §¶§b⤤ Access §cDENIED§7! §bWorld Border Module must be enabled before the size can be changed"}]}
