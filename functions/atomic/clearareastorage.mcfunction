...
execute @p[scores={clearareastorage=2},tag=stafftag] ~~~ tellraw @a {"rawtext":[{"text":"§¶§cAtomic Utility §b⤤ §d"},{"selector":"@s"},{"text":" §¶§bjust cleared storage blocks in an area"}]}
kill @e[type=armor_stand,name=clearareastorage]

#Make-Shift "are you sure"
execute @p[scores={clearareastorage=1},tag=stafftag] ~~~ tellraw @p[tag=!clearcheck] {"rawtext":[{"text":"§¶§cAtomic⤤ This will fill all storage blocks within a 15 radius with air!"}]}
execute @p[scores={clearareastorage=1},tag=stafftag] ~~~ tellraw @p[tag=!clearcheck] {"rawtext":[{"text":"§¶§cAtomic⤤ This can't be fixed! Execute command again if you're sure."}]}
execute @p[scores={clearareastorage=2},tag=stafftag] ~~~ tellraw @p[tag=!clearcheck] {"rawtext":[{"text":"§¶§cAtomic⤤ The surrounding storage blocks have been cleared with air."}]}
